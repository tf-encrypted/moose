...
pub fn well_formed(comp: Computation) -> anyhow::Result<Computation> {
    ...
    for op in &comp.operations {
        ...
        let compile_error: Option<Error> = match &op.kind {
            ...
            Broadcast(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            PrfKeyGen(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            Xor(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            And(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            Or(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            ...
            SampleSeeded(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            ...
            Reshape(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            ...
            Dot(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            Inverse(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            Add(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            Sub(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            Mul(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            Mean(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            Sum(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            Div(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            ...
            Equal(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            ...
            LessThan(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            GreaterThan(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
            ...
            Decrypt(op) => NgDispatchKernel::<SymbolicSession>::compile(op, plc).err(),
