...
impl Session for AsyncSession {
    ...
    fn execute(
        ...
        let kernel = match (op, plc) {
            ...
            (Sqrt(op), _) => DispatchKernel::compile(&op, plc)?,
            (Shl(op), _) => DispatchKernel::compile(&op, plc)?,
            (Shr(op), _) => DispatchKernel::compile(&op, plc)?,
            (Abs(op), _) => DispatchKernel::compile(&op, plc)?,
            (Diag(op), _) => DispatchKernel::compile(&op, plc)?,
        };
