...
impl Compile<Kernel> for StdConcatenateOp {
    fn compile(&self, _ctx: &CompilationContext) -> Result<Kernel> {
        ...
        match self.ty {
            ...
            Ty::Int32Tensor => closure_kernel!(vec[Int32Tensor], |xs| concatenate(axis, &xs)),
            Ty::Int64Tensor => closure_kernel!(vec[Int64Tensor], |xs| concatenate(axis, &xs)),
            Ty::Uint32Tensor => {
...
impl Compile<Kernel> for StdSliceOp {
    fn compile(&self, _ctx: &CompilationContext) -> Result<Kernel> {
        ...
        match self.ty {
            Ty::Shape => closure_kernel!(Shape, |x| x.slice(start, end)),
            _ => Err(Error::UnimplementedOperator(format!("{:?}", self))),
        }
    }
}
...
impl Compile<Kernel> for RingAddOp {
    fn compile(&self, _ctx: &CompilationContext) -> Result<Kernel> {
        match (self.lhs, self.rhs) {
            (Ty::Ring64Tensor, Ty::Ring64Tensor) => {
                function_kernel!(Ring64Tensor, Ring64Tensor, |x, y| x + y)
            }
            (Ty::Ring128Tensor, Ty::Ring128Tensor) => {
...
impl Compile<Kernel> for RingSumOp {
    fn compile(&self, _ctx: &CompilationContext) -> Result<Kernel> {
        let axis = self.axis.map(|a| a as usize);
        match self.ty {
            Ty::Ring64Tensor => closure_kernel!(Ring64Tensor, |x| x.sum(axis)),
            Ty::Ring128Tensor => closure_kernel!(Ring128Tensor, |x| x.sum(axis)),
            _ => Err(Error::UnimplementedOperator(format!("{:?}", self))),
        }
    }
}
...
impl Compile<Kernel> for RingShapeOp {
    fn compile(&self, _ctx: &CompilationContext) -> Result<Kernel> {
        match self.ty {
            Ty::Ring64Tensor => function_kernel!(Ring64Tensor, |x| x.shape()),
            Ty::Ring128Tensor => function_kernel!(Ring128Tensor, |x| x.shape()),
            _ => Err(Error::UnimplementedOperator(format!("{:?}", self))),
        }
    }
}
...
impl Compile<Kernel> for RingFillOp {
    fn compile(&self, _ctx: &CompilationContext) -> Result<Kernel> {
        match (self.ty, self.value.clone()) {
            (Ty::Ring64Tensor, Value::Ring64(value)) => {
                closure_kernel!(Shape, |shape| Ring64Tensor::fill(&shape, value))
            }
            (Ty::Ring128Tensor, Value::Ring128(value)) => {
