'''
This script benchmarks accuracy of tree enseble classification against Sklearn's random forrest classifier to test if loss of precision occurs.
...
def _build_prediction_logic(onnx_proto):
    predictor = tree_ensemble.TreeEnsembleClassifier.from_onnx(onnx_proto)
...
def benchmark(X_train, y_train, X_test, y_test):
    te = RandomForestClassifier(n_estimators=10)
    trained_model = te.fit(X_train, y_train)
...
if __name__ == '__main__':
    ...
    # save results to csv
    results.to_csv("forrest_classification_accuracy_analysis.csv")
    print(results)