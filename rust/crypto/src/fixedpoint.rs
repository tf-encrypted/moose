...
pub fn ring_encode(x: &ArrayViewD<f32>, scaling_factor: u32) -> Ring64Tensor {
    let x_upshifted = x * scaling_factor as f32;
    let x_converted: ArrayD<i64> = x_upshifted.mapv(|el| el as i64);
    Ring64Tensor::from(x_converted)
}

pub fn ring_decode(x: &Ring64Tensor, scaling_factor: u32) -> ArrayD<f32> {
    let x_upshifted: ArrayD<i64> = x.into();
    let x_converted = x_upshifted.mapv(|el| el as f32);
    x_converted / scaling_factor as f32
}
...
mod tests {
    ...
    fn ring_fixedpoint() {
        ...
        let scaling_factor = 2u32.pow(16);
