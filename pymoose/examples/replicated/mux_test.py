    ...
    def _setup_less_comp(self):
        ...
        def my_less_comp():
            with bob:
                ...
                zeros = edsl.constant(np.array([0, 0, 0, 0], dtype=np.float64))
                zeros = edsl.cast(zeros, dtype=edsl.fixed(8, 27))
            ...
            with rep:
                z_rep = edsl.less(x, y)
                yo_rep = edsl.mux(z_rep, x, y)

            with alice:
                z_host = edsl.add(yo_rep, zeros)
