...
def find_op_types_in_model_proto(model_proto, enforce=True):
    ...
    if enforce and operations is None:
        raise ValueError(f"Model proto nodes do not contain op_type.")
    return operations
...
def find_output_in_model_proto(model_proto, enforce=True):
    ...
    if enforce and output_dim is None:
        raise ValueError(f"Model proto does not contain output dimention.")
    return output_dim