...
impl Compile<SyncKernel> for Operator {
    fn compile(&self, ctx: &CompilationContext) -> Result<SyncKernel> {
        use Operator::*;
        match self {
            IdentityOp(op) => op.compile(ctx),
            LoadOp(op) => op.compile(ctx),
            SaveOp(op) => op.compile(ctx),
            SendOp(op) => op.compile(ctx),
            ReceiveOp(op) => op.compile(ctx),
            InputOp(op) => op.compile(ctx),
            OutputOp(op) => op.compile(ctx),
            ConstantOp(op) => op.compile(ctx),
            StdAddOp(op) => op.compile(ctx),
            StdSubOp(op) => op.compile(ctx),
            StdMulOp(op) => op.compile(ctx),
            StdDivOp(op) => op.compile(ctx),
            StdDotOp(op) => op.compile(ctx),
            StdMeanOp(op) => op.compile(ctx),
            StdOnesOp(op) => op.compile(ctx),
            StdConcatenateOp(op) => op.compile(ctx),
            StdExpandDimsOp(op) => op.compile(ctx),
            StdReshapeOp(op) => op.compile(ctx),
            StdAtLeast2DOp(op) => op.compile(ctx),
            StdShapeOp(op) => op.compile(ctx),
            StdSliceOp(op) => op.compile(ctx),
            StdSumOp(op) => op.compile(ctx),
            StdTransposeOp(op) => op.compile(ctx),
            StdInverseOp(op) => op.compile(ctx),
            RingAddOp(op) => op.compile(ctx),
            RingSubOp(op) => op.compile(ctx),
            RingMulOp(op) => op.compile(ctx),
            RingDotOp(op) => op.compile(ctx),
            RingSumOp(op) => op.compile(ctx),
            RingShapeOp(op) => op.compile(ctx),
            RingSampleOp(op) => op.compile(ctx),
            RingFillOp(op) => op.compile(ctx),
            RingShlOp(op) => op.compile(ctx),
            RingShrOp(op) => op.compile(ctx),
            RingInjectOp(op) => op.compile(ctx),
            BitExtractOp(op) => op.compile(ctx),
            BitSampleOp(op) => op.compile(ctx),
            BitFillOp(op) => op.compile(ctx),
            BitXorOp(op) => op.compile(ctx),
            BitAndOp(op) => op.compile(ctx),
            PrimDeriveSeedOp(op) => op.compile(ctx),
            PrimGenPrfKeyOp(op) => op.compile(ctx),
            FixedpointRingEncodeOp(op) => op.compile(ctx),
            FixedpointRingDecodeOp(op) => op.compile(ctx),
            FixedpointRingMeanOp(op) => op.compile(ctx),
